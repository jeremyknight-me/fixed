...
public sealed class FixedSerializerGenerator : IIncrementalGenerator
{
    ...
    private static void Execute(SourceProductionContext context, INamedTypeSymbol typeSymbol)
    {
        ...
        // Debugging aid: emit property metadata as comments in generated code
        //foreach (PropertyMetadata p in props)
        //{
        //    sb.AppendLine($"// {p.Name}: Type={p.TypeName}, Width={p.Width}, Alignment={p.Alignment}, Overflow={p.Overflow}, Padding='{p.PaddingCharacter}', Format='{p.StringFormat}'");
        //}
